...
from teacher.models import TeacherDocuments
...
def student_modules(request):
    student= Students.objects.get(user= request.user)
    modules= Modules.objects.filter(department= request.user.department, year=student.year, program= student.program)
...
@login_required
@has_permission
def student_module_documents(request, module_id):
    module= Modules.objects.get(id= module_id)
    documents=TeacherDocuments.objects.filter(module= module)
    return render(request, "student_module_documents.html", context={
        "documents":documents,
        "module":module
    })